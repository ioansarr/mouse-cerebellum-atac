---
title: "Mouse Cerebellum - 007 developmental differences in prenatal EGL"
...
ggplot(filter(peak_stats,genomic_class_broad %in% c("Distal", "Intronic"), is.na(temp_clust)==F), aes(x=temp_clust, y=meanPhastCons_100bp, fill=temp_clust)) +
  geom_boxplot(notch = T) +
...
ggplot(filter(peak_stats,genomic_class_broad %in% c("Distal", "Intronic"), is.na(temp_clust)==F), aes(temp_clust,  fill=as.factor(min_age_num)))+
