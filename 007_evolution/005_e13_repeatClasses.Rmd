...
```{r}
cell_i <- which(peak_mat@colData$Timepoint=="e13")
...
p <- ggplot(filter(rep_freq.gather,Cell_type_broad %in% c("Astroglia", "GC", "Interneuron", "PC", "Glut_DN", "GABA_DN")), aes(x=repFamily, y=freq, fill=Cell_type_broad)) +
  geom_boxplot(outlier.alpha = 0) +
  theme_classic() +
  scale_fill_manual(values = c("#BB4D90", "#43bbc8","#62943c", "#de761d","#d3405a", "#527cde")) +
  ylab("Fraction of counts in peaks overlapping repeat") +
  xlab("")

p

pdf("~/Mouse_Cereb/Figures/008_conservation/TE_byCellType_e13_GCPs.pdf", width = 12, height = 4, useDingbats = F); print(p); dev.off()
```

```{r}
sort(sapply(levels(rep_freq.gather$repFamily), function(r) {
  return(wilcox.test(rep_freq.gather$freq[rep_freq.gather$Cell_type_broad=="GC" & rep_freq.gather$repFamily==r],
            rep_freq.gather$freq[rep_freq.gather$Cell_type_broad %in% c("Astroglia",  "Interneuron", "PC", "Glut_DN", "GABA_DN") & rep_freq.gather$repFamily==r], alternative = "greater")$p.value)
}))

sort(sapply(levels(rep_freq.gather$repFamily), function(r) {
  mean(rep_freq.gather$freq[rep_freq.gather$Cell_type_broad=="GC" & rep_freq.gather$repFamily==r])/
            mean(rep_freq.gather$freq[rep_freq.gather$Cell_type_broad %in% c("Astroglia", "Interneuron", "PC", "Glut_DN", "GABA_DN")& rep_freq.gather$repFamily==r])
}), decreasing = T)
```
...
```{r}
saveRDS(rep_freq.gather, "~/Mouse_Cereb/016_conservation_and_repeats/Mouse_Cerebellum_e13_repeatClasses_byCell.rds")
```
