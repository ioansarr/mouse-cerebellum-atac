---
title: "Mouse Cerebellum - Peaks of developmental change across cell types"
...
Selecting cell type to be processed
...
Subsetting for particular cell type
...
```{r}
if(dir.exists(paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type))==F) {
  dir.create(paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type), recursive = T)
}
```


```{r}
cellType_mat <- makePseudobulk(cell_type = cell_type)

saveRDS(cellType_mat, paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type, "/", cell_type, "_matrix_pseudobulk_byLib.rds"))

dim(cellType_mat)

plot(colSums(cellType_mat))

## Keeping only pseudobulks with at least 1 million reads!
cellType_mat <- cellType_mat[, colSums(cellType_mat) > 1e6]

## Also checking that both replicates passed the test, otherwise remove the remaining one
timepoint_counts <- table(sapply(colnames(cellType_mat), function(x) str_split(x, "_",simplify = T)[1]))

t <- names(timepoint_counts)[timepoint_counts > 1]

cellType_mat <- cellType_mat[, grepl(paste(t, collapse = "|"), colnames(cellType_mat))]

## Now constructing a DESEq object
cellType.deseq <- DESeqDataSetFromMatrix(countData = cellType_mat[grepl("chrX", row.names(cellType_mat)) ==F,],
                                     colData = atac.info[atac.info$sample %in% colnames(cellType_mat),],
                                     design = ~ timepoint)

cellType.deseq <- DESeq(cellType.deseq)

saveRDS(cellType.deseq, paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type, "/", cell_type, "pseudobulk_byTimepoint_deseq.rds"))

cellType.timepoints <- unique(atac.info$timepoint[atac.info$sample %in% colnames(cellType_mat)])
...
saveRDS(cellType.deseq.res_list, paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type, "/", cell_type, "pseudobulk_byTimepoint_deseq_results_list.rds"))

saveRDS(cellType.deseq.res, paste0("~/Mouse_Cereb/012_devPeaks/byCellType/", cell_type, "/", cell_type, "_pseudobulk_byTimepoint_deseq_results.rds"))
...
p <- ggplot(cellType.deseq.res.gather, aes(x=stage, y=n, color=sign)) +
  geom_point() +
  geom_line(lwd = 0.8) +
  scale_x_continuous(breaks = seq(min(cellType.deseq.res.gather$stage), max(cellType.deseq.res.gather$stage), by = 1), labels=unique(cellType.deseq.res.gather$timepoint)) +
...
pdf(paste0("~/Mouse_Cereb/Figures/004_devPeaks/Mouse_peaks_", cell_type, "_deseq_byStage.pdf"), width = 6, height = 4, useDingbats = F); print(p); dev.off()
```
