...
interesting_genes <- c("Ptf1a", "Gad1", "Gad2","Skor1","Nhlh1","Kirrel2", "Foxp2", "Fgf3", "Calb1", "Cdh9", "Cdh10", "Foxp1")
...
pseudotime_root <- "sa083_Mouse_Cerebellum_e12_M#AACCGATCATGGTTTG-1"
...
pseudotime_bins <- data.frame(cell=row.names(atac.seurat@meta.data),
                              ...
                              stringsAsFactors = F) %>%
  arrange(pseudotime) %>%
  mutate(pseudotime_bin=cut(x = rank(pseudotime, ties.method = "first"), breaks = N_pseudotime_bins, labels = F)) %>%
...
FeaturePlot(atac.seurat, reduction = "umap_harmony", features = "pseudotime_bin")
...
pdf(paste0("~/Mouse_Cereb/Figures/007_withinLineages/", lineage, "/Mouse_within", lineage, "_lineage_harmony_unaligned_UMAP_pseudotime.pdf"), width = 6, height = 4, useDingbats = F); print(p); dev.off()
