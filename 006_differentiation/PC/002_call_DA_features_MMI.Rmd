...
peaks_sc_mmi <- sapply(1:nrow(peaks_scaled), function(i) {
  cmi <- cminjk.pw(v1 = unique(pseudotime_bins$mean_pseudotime), v2 = t(peaks_scaled[i,]))
})
...
peaks_sc_mmi_shuffled_multi <- do.call(cbind,parallel::mclapply(1:n_permutations, function(i) {
  ...
  pseudotime_shuffled <- sample(unique(pseudotime_bins$mean_pseudotime), length(unique(pseudotime_bins$mean_pseudotime)))
...
genes_sc_mmi <- sapply(1:nrow(genes_scaled), function(i) {
  cmi <- cminjk.pw(v1 = unique(pseudotime_bins$mean_pseudotime), v2 = t(genes_scaled[i,]))
})
...
motifs_sc_mmi <- sapply(1:nrow(motifs_scaled), function(i) {
  cmi <- cminjk.pw(v1 = unique(pseudotime_bins$mean_pseudotime), v2 = t(motifs_scaled[i,]))
})
